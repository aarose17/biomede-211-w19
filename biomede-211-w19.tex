...
\begin{enumerate}
	...
	\includegraphics{figures/single_pull_single_throw.png}
	\item Single pole, double throw, SPDT
	\includegraphics{figures/single_pull_double_throw.png}
	\item Double pole, single throw, DPST
	\includegraphics{figures/double_pull_single_throw.png}
	\item Double pole, double throw, DPDT
	\includegraphics{figures/double_pull_double_throw.png}
\end{enumerate}
...
\section{Diodes}
\begin{enumerate}
	\item Two-terminal circuit elements that allow current to flow only in one direction
	\item Are particularly useful when we want to prevent current from going to a particular portion of a circuit or direct current toward some portion of a circuit
	\item Types of diodes include light-emitting diodes, otherwise known as LEDs (which emit light as current flows through them), and photodiodes (which absorb light and convert it into electrical current)
\end{enumerate}
